...
class Repository {
    ...
    findOne(query, options = {}) {
        return rxjs_1.defer(() => this.model.findOneAsync(query, Object.assign(Object.assign({}, options), defaultOptions.findOptions))).pipe(operators_1.map(x => x && transform_entity_utils_1.transformEntity(this.target, x)));
    }
    findOneOrFail(query, maybeOptions = {}) {
        return this.findOne(query, maybeOptions).pipe(operators_1.map(entity => {
            if (entity === undefined) {
    ...
    find(query, options = {}) {
        return rxjs_1.defer(() => this.model.findAsync(query, Object.assign(Object.assign({}, options), defaultOptions.findOptions))).pipe(operators_1.map(x => transform_entity_utils_1.transformEntity(this.target, x)));
    }
    findAndCount(query, options = {}) {
        return rxjs_1.defer(() => this.model.findAsync(query, Object.assign(Object.assign({}, options), defaultOptions.findOptions))).pipe(operators_1.map(x => transform_entity_utils_1.transformEntity(this.target, x)), operators_1.map(entities => [entities, entities.length]));
    }
    save(entityLike, options = {}) {
        const saveFunc = (entity) => __awaiter(this, void 0, void 0, function* () {
            ...
            return transform_entity_utils_1.transformEntity(this.target, model.toJSON());
        });
        const saveMultipleFunc = (arrayLike) => Promise.all(arrayLike.map(x => saveFunc(x)));
        return Array.isArray(entityLike)
            ? rxjs_1.defer(() => saveMultipleFunc(entityLike))
            : rxjs_1.defer(() => saveFunc(entityLike));
    }
    update(query, updateValue, options = {}) {
        return rxjs_1.defer(() => this.model.updateAsync(query, updateValue, Object.assign(Object.assign({}, defaultOptions.updateOptions), options)));
    }
    remove(entityOrEntities, options = {}) {
        ...
        return rxjs_1.defer(() => Promise.all(promiseArray)).pipe(operators_1.map(() => entityOrEntities));
    }
    delete(query = {}, options = {}) {
        return rxjs_1.defer(() => this.model.deleteAsync(query, Object.assign(Object.assign({}, defaultOptions.deleteOptions), options)));
    }
    truncate() {
